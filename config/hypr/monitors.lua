...
local zero_monitor_scale = "auto"
hl.monitor({ output = "", mode = "preferred", position = "auto", scale = zero_monitor_scale })
...
-- sizes X11/XWayland windows, which Zero leaves unscaled so they stay
...
local zero_gdk_scale = 2
hl.env("GDK_SCALE", tostring(zero_gdk_scale))
