...
QtObject {
  ...
  // component follows the fontconfig alias `zero-font-set` writes.
  ...
  // alias path keeps working when the user runs `zero font set`), but
  ...
  // ZERO_MENU_FONT override for users who want a different family on the
  ...
  readonly property string menuFontFamily: {
    var override = Quickshell.env("ZERO_MENU_FONT")
  ...
  // Resolve the fontconfig alias to a concrete family name. `zero font
  ...
  // `zero toggle window-gaps` creates/removes this flag file. Hyprland
  ...
  property FileView windowNoGapsToggle: FileView {
    path: Quickshell.env("HOME") + "/.local/state/zero/toggles/hypr/window-no-gaps.lua"
    watchChanges: true
    printErrors: false
