...
function glyphFromHints(hints) {
  return stringHint(hints, "zero-glyph")
}

// The click action: a JSON argv string from zero-notification-send
...
function execArgvFromHints(hints) {
  return stringHint(hints, "zero-exec-argv")
}

// Validate a persisted zero-exec-argv into a runnable argv, or null. This is
...
// ~/.local/state/zero/notifications/ so toasts survive shell restarts
// (e.g. the restart `zero-update` performs). The file exists exactly as
...
// senders (all Zero web apps) delete their scoped /tmp files on close,
