// Notification service for the zero shell.
...
Item {
  ...
  readonly property string stateDir: home + "/.local/state/zero/"
  ...
  // Toasts are fixed to the top-right corner. They only clear the zero bar
  ...
  PersistentProperties {
    id: persisted
    reloadableId: "zero-notifications"
  ...
  // and rare.
  //   - zero-action: a user-action confirmation toast ("Theme changed",
  ...
  //     Trusted because it's almost always zero or system shell scripts —
  ...
  //   - app_name is "zero-action" (Zero's own user-action toasts —
  ...
  // Run the popup's click action, then dismiss. Zero's own toasts carry the
  ...
  function focusApp(entry) {
    ...
    focusAppProc.command = [
      service.zeroPath + "/bin/zero-hyprland-focus-app",
      String(entry.app)
    ]
  ...
  // toasts survive shell restarts (notably the restart `zero-update`
  ...
  Variants {
    ...
    PanelWindow {
      ...
      WlrLayershell.namespace: "zero-notifications"
