...
Panel {
  id: root
  moduleName: "zero.bluetooth"
  ipcTarget: "zero.bluetooth"
  // manageIpc: false so this panel can own the single IpcHandler the target
  // permits — needed for the toggleBluetooth method below.
  manageIpc: false

  // Address -> "connecting" | "disconnecting" | "forgetting".
  // The actual Bluetooth sequencing lives in bin/zero-bluetooth-device;
  ...
  function deviceCommand(action, address) {
    return ["zero-bluetooth-device", action, address]
  }
  ...
  // disconnects, then removes the BlueZ pairing record via zero-bluetooth-device.
  ...
  // the adapter came back on at the next boot. zero-bluetooth-power moves the
  ...
  function toggleBluetooth() {
    if (!adapter) return
    Quickshell.execDetached(["zero-bluetooth-power", adapter.enabled ? "off" : "on"])
  }

  IpcHandler {
    target: "zero.bluetooth"
