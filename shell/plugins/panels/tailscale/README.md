# Tailscale Zero Widget

Native Zero bar widget for Tailscale.
...
`zero-tailscale-receive` service, which announces each one with a
notification (an image preview when the file is an image, and a click to open
it). The Tailscale service install enables it; `zero tailscale receive`
...
This widget ships as first-party plugin `zero.tailscale`. Add it with `zero plugin enable zero.tailscale`, then place it with `zero bar move zero.tailscale` if desired.
