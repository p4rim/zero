...
Panel {
  id: root
  moduleName: "zero.power"
  ipcTarget: "zero.power"
  ...
  // Status phrases rotate while current is flowing in either direction.
  ...
  function setProfile(profile) {
    ...
    actionProc.command = ["zero-powerprofiles-set", root.discharging ? "battery" : "ac", profile]
  ...
  Process {
    id: batteryProc
    command: ["zero-battery-status", "--shell"]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.updateKeyValue(text, "battery") }
  }

  Process {
    id: profilesProc
    command: ["zero-powerprofiles-list", "--active-state"]
    stdout: StdioCollector { waitForEnd: true; onStreamFinished: root.updateProfiles(text) }
  }

  Process {
    id: systemProc
    command: ["zero-system-stats"]
