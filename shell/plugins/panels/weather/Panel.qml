...
Panel {
  id: root
  moduleName: "zero.weather"
  ipcTarget: "zero.weather"
  manageIpc: false
  ...
  // zero-weather-location). The query is the wttr.in path segment
  ...
  property FileView locationFile: FileView {
    path: Quickshell.env("HOME") + "/.local/state/zero/settings/weather.json"
    watchChanges: true
    printErrors: false
  ...
  function persistLocation(name, latitude, longitude) {
    if (name && latitude !== null && longitude !== null)
      locationSaveProc.command = ["zero-weather-location", "--set", name, latitude + "," + longitude]
    else if (name)
      locationSaveProc.command = ["zero-weather-location", "--set", name]
    else
      locationSaveProc.command = ["zero-weather-location", "--clear"]
  ...
  // Mirrors zero-weather-icon's wttr.in code → nerd-font glyph mapping.
