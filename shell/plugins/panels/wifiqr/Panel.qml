...
// zero-network-qr, which emits the interface, security, and SSID it
...
Item {
  ...
  function dismiss() {
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "zero.wifiqr")
    else close()
  }
  ...
  function generate(requestedIface) {
    ...
    qrProc.command = requestedIface
      ? ["zero-network-qr", "--meta", requestedIface]
      : ["zero-network-qr", "--meta"]
    qrProc.running = true
  }
  ...
  function togglePassword() {
    ...
    pwProc.command = ["zero-network-password", iface]
    pwProc.running = true
  }
  ...
  PanelWindow {
    ...
    WlrLayershell.namespace: "zero-network-qr"
