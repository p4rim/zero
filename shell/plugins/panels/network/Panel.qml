...
Panel {
  id: root
  moduleName: "zero.network"
  ipcTarget: "zero.network"
  ...
  // `zero-network-status --verbose` samples (~1.5s apart via detailsPoll).
  ...
  // Wi-Fi band state from `zero-network-band`. `bandCurrent` is the band
  ...
  function openCaptivePortal() {
    ...
    Quickshell.execDetached(["zero-launch-browser", Model.captivePortalUrl])
    close()
  }
  ...
  // The share card is its own panel plugin (zero.wifiqr) so a replacement
  ...
  function summonWifiQr(forceDetect) {
    ...
    bar.shell.summon("zero.wifiqr", JSON.stringify(payload))
  }
  ...
  function refresh(scanWifi) {
    ...
    if (!bandProc.running) {
      bandProc.command = ["zero-network-band"]
  ...
  function setBand(band) {
    ...
    actionProc.command = ["zero-network-band", band]
    actionProc.running = true
  }

  // The speed test is its own panel plugin (zero.speedtest) so a
  ...
  function summonSpeedTest() {
    ...
    bar.shell.summon("zero.speedtest", connection ? JSON.stringify({ connection: connection }) : "{}")
  }

  function dnsCommand(provider) {
    var command = "zero-dns"
  ...
  function setDns(provider) {
    ...
    if (provider === "Custom") {
      var launcher = "zero-launch-floating-terminal-with-presentation"
  ...
  Process {
    id: detailsProc
    command: ["zero-network-status", "--verbose"]
