...
Panel {
  id: root
  moduleName: "zero.monitor"
  ipcTarget: "zero.monitor"
  manageIpc: false
  ...
  // these stops; the CLI (zero-display-text-size) accepts any integer in range.
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["zero-brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function showBrightnessOsd(percent) {
    ...
    bar.shell.summon("zero.osd", JSON.stringify({
      icon: "brightness",
      value: percent
    }))
  }
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", "zero-hyprland-monitor-scaling " + scale]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = ["zero-display-text-size", String(px)]
  ...
  Process {
    ...
    // `zero-brightness-display` races the hardware/driver and can
