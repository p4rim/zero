...
// against `zero-default-browser` -- so the batch runs it once and the rows
...
var GUARD_READERS = [
  "zero-default-browser",
  "zero-default-editor",
  "zero-default-terminal",
  "zero-dns"
]
...
function guardHelpers() {
  return 'declare -A __zero_pkgs=()\n'
    + 'mapfile -t __zero_pkg_names < <({ pacman -Qq; LC_ALL=C pacman -Qi'
    + " | awk '/^[A-Za-z]/ { provides = ($0 ~ /^Provides/); sub(/^[^:]*: /, \"\") }"
    + ' provides && $0 != "None" { n = split($0, p, " ");'
    + ' for (i = 1; i <= n; i++) { sub(/[<>=].*/, "", p[i]); print p[i] } }\'; } 2>/dev/null)\n'
    + 'for __zero_pkg in "${__zero_pkg_names[@]}"; do __zero_pkgs[$__zero_pkg]=1; done\n'
    + '__zero_pkg_has() { [[ -n ${__zero_pkgs[$1]-} ]] && return 0; '
    + '[[ $1 == *[\\<\\>=]* ]] && { pacman -Q "$1" &>/dev/null; return; }; return 1; }\n'
    + 'zero-pkg-present() { local p; for p in "$@"; do __zero_pkg_has "$p" || return 1; done; return 0; }\n'
    + 'zero-pkg-missing() { local p; for p in "$@"; do __zero_pkg_has "$p" || return 0; done; return 1; }\n'
    + 'zero-cmd-present() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 1; done; return 0; }\n'
    + 'zero-cmd-missing() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 0; done; return 1; }\n'
}
...
function guardPrelude(guards) {
  ...
  for (var i = 0; i < GUARD_READERS.length; i++) {
    ...
    prelude += "__zero_read_" + i + "=$(" + GUARD_READERS[i] + " 2>/dev/null) || :\n"
  }
