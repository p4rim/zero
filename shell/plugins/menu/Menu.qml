...
Item {
  ...
  // Injected by zero-shell when this plugin is summoned.
  property string zeroPath: Quickshell.env("ZERO_PATH")
  property var shell: null
  property var manifest: null

  // Plugin lifecycle hooks. The host calls open(payloadJson) after
  // `zero-shell shell summon zero.menu ...` and close() when hidden.
  ...
  property string defaultMenuPath: zeroPath + "/config/zero/zero-menu.jsonc"
  property string userMenuPath: Quickshell.env("HOME") + "/.config/zero/extensions/zero-menu.jsonc"
  ...
  readonly property var providers: ({
    "fonts": {
      script: "current=$(zero-font-current 2>/dev/null); zero-font-list 2>/dev/null | while read -r f; do [[ -z $f ]] && continue; printf '%s\\t%s\\t%s\\n' \"$f\" \"$f\" \"$current\"; done",
      icon: "",
      volatile: true,
      actionFor: function(value) { return "zero-font-set " + Util.shellQuote(value) }
    },
    "power-profiles": {
      script: "current=$(powerprofilesctl get 2>/dev/null); zero-powerprofiles-list 2>/dev/null | while read -r p; do [[ -z $p ]] && continue; printf '%s\\t%s\\t%s\\n' \"$p\" \"$p\" \"$current\"; done",
      icon: "\udb81\udc0b",
      actionFor: function(value) { return "zero-powerprofiles-set autodetect " + Util.shellQuote(value) }
    }
  })
  ...
  // `zero-shell shell summon zero.menu '{"menu":"system"}'`.
  ...
  function openRoute(initialMenu) {
    ...
    // a leaf, e.g. `zero menu summon screenrecord-stop`), run it directly
  ...
  // user extension at ~/.config/zero/extensions/zero-menu.jsonc) take
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "zero-menu"
