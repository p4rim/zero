...
Item {
  ...
  // Injected by zero-shell; defaults to the session ZERO_PATH.
  property string zeroPath: Quickshell.env("ZERO_PATH")
  property string stateHome: Quickshell.env("HOME") + "/.local/state"
  property string imageDirs: Quickshell.env("ZERO_IMAGE_SELECTOR_DIRS") || Quickshell.env("ZERO_IMAGE_SELECTOR_DIR") || Quickshell.env("ZERO_STOCK_BACKGROUNDS_DIR") || (stateHome + "/zero/current/theme/backgrounds")
  property string imageRows: ""
  property string loadedImageRows: ""
  property string selectionFile: Quickshell.env("ZERO_IMAGE_SELECTOR_SELECTION_FILE") || Quickshell.env("ZERO_BACKGROUND_SELECTION_FILE")
  property string selectedImage: Quickshell.env("ZERO_IMAGE_SELECTOR_SELECTED")
  ...
  function scriptPath(name) {
    return zeroPath + "/shell/plugins/image-picker/" + name
  }
  ...
  // Lifecycle hooks invoked by zero-shell summon/hide. shell.summon(id,
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "zero-image-selector"
