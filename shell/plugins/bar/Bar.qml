...
Item {
  ...
  // The zero-shell host injects zeroPath from ZERO_PATH.
  required property string zeroPath
  ...
  property string zeroConfigDir: home + "/.config/zero"
  ...
  // to "monospace"), so changing the system font (via `zero-font-set`)
  ...
  // Resolve the live bar-widget instance for a plugin id (e.g. "zero.bluetooth").
  ...
  function customModuleSource(entry) {
    var source = BarModel.customModulePath(entry, home, zeroConfigDir)
  ...
  FileView {
    path: root.stateHome + "/zero/current"
    watchChanges: true
    printErrors: false
  ...
  // and the flag is created/removed by `zero-toggle-bar`.
  Process {
    id: barHiddenProbe
    running: true
    command: ["bash", "-c", "[[ -f $HOME/.local/state/zero/toggles/bar-off ]] && echo yes || echo no"]
    stdout: SplitParser { onRead: function(line) { root.barHidden = String(line).trim() === "yes" } }
  }
  FileView {
    path: root.home + "/.local/state/zero/toggles"
    watchChanges: true
    printErrors: false
  ...
  // shell restarts. `zero-toggle-bar` nudges this after flipping the flag
  ...
  component BarPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "zero-bar"
  ...
  component DragGhostPanel: PanelWindow {
    ...
    WlrLayershell.namespace: "zero-bar-drag-ghost"
  ...
  component BarMoveGhostPanel: PanelWindow {
    ...
    color: "transparent"
    exclusionMode: ExclusionMode.Ignore
    WlrLayershell.namespace: "zero-bar-move-ghost"
    WlrLayershell.layer: WlrLayer.Overlay
    WlrLayershell.keyboardFocus: WlrKeyboardFocus.None
