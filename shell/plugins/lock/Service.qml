...
Item {
  ...
  property string zeroPath: ""

  readonly property string home: Quickshell.env("HOME")
  readonly property string stateHome: home + "/.local/state"
  readonly property string userName: Quickshell.env("USER") || Quickshell.env("LOGNAME")
  readonly property string currentBackgroundLink: stateHome + "/zero/current/background"
  ...
  function logEvent(event) {
    ...
    console.log("zero lock " + lastEventAt + " " + event)
  }
  ...
  PanelWindow {
    ...
    color: "transparent"
    WlrLayershell.namespace: "zero-lock-preview"
  ...
  Process {
    ...
    command: ["bash", "-c", "if [[ -f /etc/pam.d/zero-lock-fingerprint ]] && command -v fprintd-list >/dev/null 2>&1 && fprintd-list \"$USER\" 2>/dev/null | grep -qi finger; then echo yes; else echo no; fi"]
  ...
  Process {
    ...
    command: ["bash", "-c", "zero-hyprland-session-locked"]
  ...
  Process {
    id: wakeProcess
    command: ["bash", "-c", "zero-system-wake"]
  }

  Process {
    id: blankProcess
    command: ["bash", "-c", "zero-brightness-keyboard off; zero-brightness-display off"]
  }
  ...
  FileView {
    path: "/etc/pam.d/zero-lock-password"
    watchChanges: true
    printErrors: false
