...
Item {
  ...
  // Injected by zero-shell (the first-party service loader).
  property var shell: null

  readonly property string home: Quickshell.env("HOME")
  readonly property string stayAwakeStateDir: home + "/.local/state/zero/indicators"
  ...
  readonly property string screensaverClass: "org.zero.screensaver"
  ...
  function logEvent(event, details) {
    ...
    console.log("zero idle " + root.lastEventAt + " " + root.lastEvent)
  }
  ...
  function launchScreensaver() {
    ...
    runProcess(screensaverProcess, "screensaver", "[[ $(zero-shell lock isLocked 2>/dev/null) == \"true\" ]] || zero-launch-screensaver")
  }
  ...
  function lockSystem(reason) {
    ...
    runProcess(lockProcess, "lock", "zero-system-lock")
  }
  ...
  function cancelIdleCycle(reason) {
    ...
    if (root.idledThisCycle) runProcess(wakeProcess, "wake", "zero-system-wake")
  ...
  function persistStayAwake(value) {
    var command = value
      ? "mkdir -p \"$HOME/.local/state/zero/indicators\" && touch \"$HOME/.local/state/zero/indicators/stay-awake\""
      : "rm -f \"$HOME/.local/state/zero/indicators/stay-awake\""
  ...
  Process {
    ...
    command: ["bash", "-c", "mkdir -p \"$HOME/.local/state/zero/indicators\"; if [[ -f $HOME/.local/state/zero/indicators/stay-awake ]]; then echo yes; else echo no; fi"]
    stdout: SplitParser {
