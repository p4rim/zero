...
Item {
  ...
  // Maps an icon name to a file in the configured application icon directory.
  ...
  function remove(desktopId, name) {
    var id = String(desktopId || "")
    if (!id) return
    Util.execDetached(Util.shellQuote(root.zeroPath + "/bin/zero-remove-launcher-entry") + " " + Util.shellQuote(id) + " " + Util.shellQuote(String(name || id)))
  }
  ...
  function hiddenEntryScanCommand() {
    ...
    var script = root.zeroPath + "/shell/services/hidden-entries.sh"
  ...
  function closeLaunchFeedback(serial) {
    ...
    if (root.launchOsdOpen) {
      Quickshell.execDetached(["zero-shell", "osd", "close"])
  ...
  FileView {
    path: root.zeroPath + "/default/zero/launcher.hides"
    watchChanges: true
    printErrors: false
  ...
  Timer {
    ...
    onTriggered: {
      ...
      Quickshell.execDetached(["zero-shell", "osd", "show", JSON.stringify({ icon: "󱓞", message: root.launchOsdMessage, duration: 0 })])
    }
  }
