...
QtObject {
  ...
  property string pluginsDir: home + "/.config/zero/plugins"
  ...
  //   - the built-in bar option (`zero.bar`) is active when `bar.id` is
  //     missing or set to `zero.bar`.
  ...
  //     silently make `zero launch bar-settings` a no-op. Turning one off
  ...
  function isEnabled(id) {
    ...
    if (manifest) {
      if (Array.isArray(manifest.kinds) && manifest.kinds.indexOf("bar") !== -1) {
        ...
        if (!selectedBar) selectedBar = "zero.bar"
  ...
  function resolveEnabledId(id) {
    ...
    for (var candidate in installedPlugins) {
      ...
      var metadata = manifest && Util.isPlainObject(manifest.zero) ? manifest.zero : null
  ...
  // back, and so a plugin that is both a widget and a menu (zero.menu)
  ...
  function barTarget(config, placement, fallbackSection) {
    ...
    var anchors = { left: "zero.workspaces", center: "zero.weather", right: "zero.tray" }
  ...
  function activeCloneFor(config, sourceId) {
    for (var candidate in installedPlugins) {
      ...
      var candidateMetadata = candidateManifest && Util.isPlainObject(candidateManifest.zero)
        ? candidateManifest.zero : null
  ...
  function restoreCloneSource(config, cloneId, sourceId) {
    ...
    if (isBarOption) {
      if (sourceId === "zero.bar") delete config.bar.id
    ...
    shellConfigMutator(function(config) {
      ...
      if (isBarOption) {
        if (value) {
          ...
          if (clonedFrom && clonedFrom !== "zero.bar") config.bar.id = clonedFrom
  ...
  function parseScanOutput(text) {
    ...
    // `zero.*` namespace is reserved for built-ins, including bar widgets
    // registered outside the manifest-based plugin registry.
    for (var tk in thirdParty) {
      if (firstParty[tk] || String(tk).indexOf("zero.") === 0) {
        console.warn("PluginRegistry: plugin " + tk
          + " rejected: id is reserved for first-party Zero plugins")
        continue
      }
  ...
  function rescan() {
    ...
    // Third-party plugins stay at the top level of ~/.config/zero/plugins.
    var script = ""
