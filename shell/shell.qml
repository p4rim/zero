...
ShellRoot {
  ...
  // The zero-shell host is the long-running entry point. Plugins live in
  // sibling directories under plugins/. ZERO_PATH is provided by the uwsm
  // session environment and is the single source of truth for this checkout.
  property string zeroPath: Quickshell.env("ZERO_PATH")
  readonly property string shellPath: zeroPath + "/shell"
  readonly property string firstPartyPluginsDir: shellPath + "/plugins"
  readonly property string defaultsPath: zeroPath + "/config/zero/shell.json"
  readonly property string userConfigPath: home + "/.config/zero/shell.json"
  ...
  readonly property var builtinShellConfig: ({
    ...
    bar: {
      ...
      layout: {
        left: [{ id: "zero.menu" }, { id: "zero.workspaces" }],
        center: [{ id: "zero.clock", format: "dddd HH:mm" }],
        right: [{ id: "zero.audio" }]
      }
    },
    plugins: []
  ...
  Component.onCompleted: {
    console.log("zero-shell paths",
      "zeroPath=" + shell.zeroPath,
  ...
  readonly property string defaultBarId: "zero.bar"
  ...
  function configureBar(target, manifest) {
    if (!target) return
    if ("zeroPath" in target) target.zeroPath = shell.zeroPath
  ...
  function ensureService(pluginId) {
    ...
    function finalize() {
      ...
      if ("zeroPath" in inst) inst.zeroPath = shell.zeroPath
  ...
  // Destroying zero.lock drops the ext-session-lock client while Hyprland
  ...
  function isBarWidgetPanelPlugin(pluginId) {
    ...
    // panel loader (e.g. zero.menu); let that path handle them.
  ...
  Instantiator {
    ...
    delegate: QtObject {
      ...
      property Loader panelLoader: Loader {
        ...
        onLoaded: {
          if (!item) return
          if ("zeroPath" in item) item.zeroPath = shell.zeroPath
  ...
  function imagePickerItem() {
    var loader = panelLoaders["zero.image-picker"]
  ...
  IpcHandler {
    ...
    function open(imageDirs: string,
      ...
      return shell.summon("zero.image-picker", payload) ? "ok" : "unknown"
    }
