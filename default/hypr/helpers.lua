...
local function command_from(value, description)
  ...
  elseif value.tui then
    if value.focus then
      return "zero-launch-or-focus-tui " .. shell_quote(value.tui)
    else
      return "zero-launch-tui " .. shell_quote(value.tui)
    end
  end
...
function o.launch_webapp(url)
  return "zero-launch-webapp " .. shell_quote(url)
end

function o.launch_webapp_sole(name, url)
  return "zero-launch-or-focus-webapp " .. shell_quote(name) .. " " .. shell_quote(url)
end

function o.launch_sole(match, command)
  return "zero-launch-or-focus " .. shell_quote(match) .. " " .. shell_quote(o.launch(command))
end

function o.bind_toggle(keys, description, toggle, options)
  o.bind(keys, description, "zero-toggle-" .. toggle, options)
end

function o.notify(message)
  return "zero-notification-send -u low " .. shell_quote(message)
end
